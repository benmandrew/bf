...
/// WebAssembly build. Each takes raw Brainfuck source (non-command characters
...
/// Compile Brainfuck source to LLVM IR text.
/// @param source Raw Brainfuck source; cleaned internally.
/// @param optimise Run the LLVM optimisation pipeline.
/// @param label_blocks Append each block's Brainfuck source span to its name.
...
/// Compile Brainfuck source to the Graphviz dot of its control-flow graph.
/// @param source Raw Brainfuck source; cleaned internally.
